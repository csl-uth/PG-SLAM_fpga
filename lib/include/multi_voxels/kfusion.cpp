...
void KFusion::vitisRelease() {
	...
	clFinish(q);
			...
			if (err != CL_SUCCESS)
				...
				err = clEnqueueUnmapMemObject(q_intkf,volDim_buffer_kf,inDim_kf,0,NULL,NULL);
				if(err != CL_SUCCESS){
...
#ifdef PREF_FUSEVOL
#ifndef FUSE_HW
void fuseVolumesKernelPrefetched(	Volume srcVol, 
									float *tsdfBuff, 
									const sMatrix4 pose,
									const float3 origin,
...
void KFusion::integrateKernel(Volume vol, const float* depth, const uchar3* rgb, uint2 depthSize,
	...
	int end;
		...
		for (int i = 0; i < CU; i++) {
				...
				pt_intkf_in[i][1] = volDim_kf_buffer[i];
				pt_intkf_in[i][2] = integrate_depth_kf_buffer[i];
				pt_intkf_in[i][3] = InvTrack_data_kf_buffer[i];
				pt_intkf_in[i][4] = K_data_kf_buffer[i];
			#else
				pt_intkf_in[i][0] = volSize_buffer[i];
				pt_intkf_in[i][1] = volDim_buffer[i];
				pt_intkf_in[i][2] = integrate_depth_buffer[i];
				pt_intkf_in[i][3] = InvTrack_data_buffer[i];
				pt_intkf_in[i][4] = K_data_buffer[i];
			#endif
		}
		...
		for (int i = 0; i < CU; i++) {
			...
			#endif
			err |= clSetKernelArg(krnl_integrateKernel[i], argcounter++, sizeof(int), &offset);
			err |= clSetKernelArg(krnl_integrateKernel[i], argcounter++, sizeof(int), &end);
			#ifdef INT_LP6
				err |= clSetKernelArg(krnl_integrateKernel[i], argcounter++, sizeof(int), &startFlag_krnl);
			#else
				err |= clSetKernelArg(krnl_integrateKernel[i], argcounter++, sizeof(int), &startFlag_kf);
			#endif
		...
		clWaitForEvents(CU, flagDone);
			...
			pt_intkf_in[1] = volDim_kf_buffer;
			pt_intkf_in[2] = integrate_depth_kf_buffer;
			pt_intkf_in[3] = InvTrack_data_kf_buffer;
			pt_intkf_in[4] = K_data_kf_buffer;
		#else
			pt_intkf_in[0] = volSize_buffer;
			pt_intkf_in[1] = volDim_buffer;
			pt_intkf_in[2] = integrate_depth_buffer;
			pt_intkf_in[3] = InvTrack_data_buffer;
			pt_intkf_in[4] = K_data_buffer;
		#endif
		...
		err |= clSetKernelArg(krnl_integrateKernel, argcounter++, sizeof(int), &startFlag_kf);
...
void KFusion::integrateHWKernel(Volume vol, const float* depth, const uchar3* rgb, uint2 depthSize,
	...
	if (err != CL_SUCCESS) {
		...
		for (int i = 0; i < CU; i++) {
			...
			pt_int_in[i][1] = volDim_buffer[i];
			pt_int_in[i][2] = integrate_depth_buffer[i];
			pt_int_in[i][3] = InvTrack_data_buffer[i];
			pt_int_in[i][4] = K_data_buffer[i];
		}
		...
		for (int i = 0; i < CU; i++) {
			...
			err |= clSetKernelArg(krnl_integrateKernel[i], argcounter++, sizeof(int), &end);
				...
				err |= clSetKernelArg(krnl_integrateKernel[i], argcounter++, sizeof(int), &startFlag_krnl);
			#else
				err |= clSetKernelArg(krnl_integrateKernel[i], argcounter++, sizeof(int), &startFlag);
			#endif
		...
		pt_int_in[1] = volDim_buffer;
		pt_int_in[2] = integrate_depth_buffer;
		pt_int_in[3] = InvTrack_data_buffer;
		pt_int_in[4] = K_data_buffer;
		...
		if (ratio != 1) {
			...
			for (y = 0; y < params.computationSize.y; y++)
				for (unsigned int x = 0; x < params.computationSize.x; x++) {
					...
					pad_depth[x+2 + (y+2)*(padSize.x)] = inputDepth[x*ratio + y*ratio*params.inputSize.x];
				}
			/********* fill rows ************/
			memcpy(pad_depth  + 2, rawDepth, params.computationSize.x*sizeof(float));
			memcpy(pad_depth  + 2 + padSize.x , rawDepth, params.computationSize.x*sizeof(float));
			memcpy(pad_depth  + 2 + padSize.x *(padSize.y-2), rawDepth+params.computationSize.x*(params.computationSize.y-1), params.computationSize.x*sizeof(float));
			memcpy(pad_depth  + 2 + padSize.x *(padSize.y-1), rawDepth+params.computationSize.x*(params.computationSize.y-1), params.computationSize.x*sizeof(float));
			...
			for(uint y = 0; y < params.inputSize.y; y++){
				for(uint x = 0; x < params.inputSize.x; x++){
					pad_depth[x+2 + (y+2)*(padSize.x)] = inputDepth[x + y*params.inputSize.x];
				}
			}
			/********* fill rows ************/
			memcpy(pad_depth  + 2, inputDepth, params.inputSize.x*sizeof(float));
			memcpy(pad_depth  + 2 + padSize.x , inputDepth, params.inputSize.x*sizeof(float));
			memcpy(pad_depth  + 2 + padSize.x *(padSize.y-2), inputDepth+params.inputSize.x*(params.inputSize.y-1), params.inputSize.x*sizeof(float));
			memcpy(pad_depth  + 2 + padSize.x *(padSize.y-1), inputDepth+params.inputSize.x*(params.inputSize.y-1), params.inputSize.x*sizeof(float));
