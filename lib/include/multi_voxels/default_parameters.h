...
const float default_icp_threshold = 5.0e-01;
const int default_compute_size_ratio = 1;
const int default_integration_rate = 1;
...
const float3 default_initial_pos = make_float3(4.0,4.0,4.0);
...
const int default_image_freq = 30;		// rate of rgb image 
const float default_min_depth = 0.1;	// minimum depth sensed by the sensor
const float default_max_depth = 8.0;	// maximum depth sensed by the sensor
const int default_odom_freq = 30;		// rate of Kinect fusion odometry
const int default_max_num_kfs = 5; 		// Optimize when 5 new Keyframes are available
const int default_max_num_fts = 500;	// Maximum Number of Features to detect (if exist in the RGB Image)
const float default_knn_match_ratio = 0.75;	// KNN Distance ration for matching features between two keyframes
const int default_min_num_matches = 65;		// Minimum number of matches that must be detected in two keyframes in order for features to be added in pose graph 
const int default_g2o_max_iter = 6;		// Maximum iterations of pose graph optimization.
const float default_g2o_ftsWeight = 0.0005;	// Weight used for matches in pose graph optimization the greater the number the more important the information added.
const int default_kf_rate = 50;   // Key frame rate
const int default_blurr_threshold = 100;
...
static std::string short_options = "c:d:e:f:g:i:l:m:n:k:o:p:q:r:s:t:u:v:w:x:y:z:a:h:X:";

static struct option long_options[] =
  {
			{"compute-size-ratio",     required_argument, 0, 'c'},
			{"dump-volume",  		   required_argument, 0, 'd'},
			{"max-num-kfs",			   required_argument, 0, 'e'},
			{"fps",  				   required_argument, 0, 'f'},
			{"max-num-fts",			   required_argument, 0, 'g'},
			{"input-file",  		   required_argument, 0, 'i'},
			{"camera",  			   required_argument, 0, 'k'},
			{"icp-threshold", 	 	   required_argument, 0, 'l'},
			{"log-file",  			   required_argument, 0, 'o'},
			{"mu", 			 		   required_argument, 0, 'm'},
			{"knn-match-ratio",		   required_argument, 0, 'n'},
			...
			{"min-num-matches",		   required_argument, 0, 'u'},
			{"volume-resolution",      required_argument, 0, 'v'},
			{"g2o-fts-weight",         required_argument, 0, 'w'},
			{"g2o-max-iter",		   required_argument, 0, 'x'},
			{"pyramid-levels", 		   required_argument, 0, 'y'},
			{"rendering-rate", required_argument, 0, 'z'},
			{"blurr-threshold", required_argument, 0, 'h'},
			{"kf-rate", required_argument, 0, 'a'},
...
struct Configuration {
	...
	void print_arguments() {
		...
		std ::cerr << "-e  (--max-num-kfs)				: default is " << default_max_num_kfs << std::endl;
		std ::cerr << "-g  (--max-num-fts)				: default is " << default_max_num_fts << std::endl;
		...
		std ::cerr << "-n  (--knn-match-ratio)          : default is " << default_knn_match_ratio << "               " << std::endl;
		std ::cerr << "-p  (--init-pose)                : default is " << default_initial_pos.x << "," << default_initial_pos.y << "," << default_initial_pos.z << "     " << std::endl;
		...
		std ::cerr << "-u  (--min-num-matches)			: default is " << default_min_num_matches << std::endl;
		...
		std ::cerr << "-w  (--g2o-fts-weight)			: default is " << default_g2o_ftsWeight << std::endl;
		std ::cerr << "-x  (--g2o-max-iter)				: default is " << default_g2o_max_iter << std::endl;
		std ::cerr << "-y  (--pyramid-levels)           : default is 10,5,4     " << std::endl;
		std ::cerr << "-z  (--rendering-rate)   : default is " << default_rendering_rate << std::endl;
		std ::cerr << "-a  (--kf-rate)   : default is " << default_kf_rate << std::endl;
		std ::cerr << "-h  (--blurr-threshold)   : default is " << default_blurr_threshold << std::endl;
		std ::cerr << "-X  (--xclbin)   : Vitis hardware binary " <<  std::endl;
	}
...
time_t rawtime;
		...
		out << "init-pose: " << initial_pos.x << "," << initial_pos.y << "," <<initial_pos.z << std::endl;
	...
	Configuration(unsigned int argc, char ** argv) {
		...
		initial_pos = default_initial_pos;
		...
		image_freq = default_image_freq;
		min_depth = default_min_depth;
		max_depth = default_max_depth;
		odom_freq = default_odom_freq;
		max_num_kfs = default_max_num_kfs;
		max_num_fts = default_max_num_fts;
		knn_match_ratio = default_knn_match_ratio;
		min_num_matches = default_min_num_matches;
		g2o_max_iter = default_g2o_max_iter;
		g2o_ftsWeight = default_g2o_ftsWeight;
		kf_rate = default_kf_rate;
		blurr_threshold = default_blurr_threshold;
		...
		while ((c = getopt_long(argc, argv, short_options.c_str(), long_options,
			...
			case 'm':   // -m  (--mu)
				...
				break;
			case 'n':   // -n  (--knn-match-ratio)
				this->knn_match_ratio = atof(optarg);
				std::cerr << "update knn_match_ratio to " << this->knn_match_ratio << std::endl;
				break;
			case 'e':   // -e  (--max-num-kfs)
				this->max_num_kfs = atoi(optarg);
				std::cerr << "update max_num_kfs to " << this->max_num_kfs << std::endl;
				break;
			case 'g':   // -g  (--max-num-fts)
				this->max_num_fts = atoi(optarg);
				std::cerr << "update max_num_fts to " << this->max_num_fts << std::endl;
				break;
			case 'p':    //   -p  (--init-pose)
				this->initial_pos = atof3(optarg);
				std::cerr << "update init_poseFactors to "
						<< this->initial_pos.x << ","
						<< this->initial_pos.y << ","
						<< this->initial_pos.z << std::endl;
				break;
			case 'q':
				this->no_gui = true;
				break;
			...
			case 's':    //   -s  (--map-size)
				...
				break;
			case 'w':    //   -w  (--g2o-fts-weight)
				this->g2o_ftsWeight = atof(optarg);
				std::cerr << "update g2o_ftsWeight to " << this->g2o_ftsWeight
						<< std::endl;
				break;
			case 'u':    //   -u  (--min-num-matches)
				this->min_num_matches = atoi(optarg);
				std::cerr << "update min_num_matches to " << this->min_num_matches
						<< std::endl;
				break;
			case 'x':    //   -x  (--g2o-max-iter)
				this->g2o_max_iter = atoi(optarg);
				std::cerr << "update g2o_max_iter to " << this->g2o_max_iter
						<< std::endl;
				break;
			...
			case 'z':    //   -z  (--rendering-rate)
				...
				break;
			case 'a':    //   -a  (--kf-rate)
				this->kf_rate = atoi(optarg);
				std::cerr << "update kf_rate to " << this->kf_rate
						<< std::endl;
				break;
			case 'h':    //   -h  (--blurr-threshold)
				this->blurr_threshold = atoi(optarg);
				std::cerr << "update blurr-threshold to " << this->blurr_threshold
						<< std::endl;
				break;
