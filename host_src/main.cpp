...
int main(int argc, char **argv)
{
	...
	#ifdef ENABLED_POWERMONITOR
		std::atomic<bool> powermonitorIsRunning;

		// Start power monitoring thread
		std::thread powermonitor_thread(powermonitor_start,&powermonitorIsRunning);
	#endif
	...
	*logstream
	...
	while(readNextRGBDimage(&newImageSize)) {
		...
		*logstream  << frame << "\t\t" << timings[1] - timings[0] << "\t" //  acquisition
					...
					<< (timings[4] - timings[3]) + (timings[6] - timings[5]) << "\t"     //  integration
					<< timings[7] - timings[6] << "\t"     //  raycasting
					<< (timings[7] - timings[1]) - (timings[5] - timings[4]) << "\t"     //  kfusion-total
					<< (timings[8] - timings[7]) + (timings[5] - timings[4]) << "\t"     //  g2o-keyFrame
	...
	#ifdef ENABLED_POWERMONITOR
		powermonitorIsRunning.store(false); // close power monitor
		powermonitor_thread.join();
	#endif
