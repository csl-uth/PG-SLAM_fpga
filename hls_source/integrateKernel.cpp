...
extern "C"{
#ifdef INT_HP // fp16
void integrateKernel(uint4 *size, floatH4 *vg_ratio, 
	...
	floatH maxweight, uint start, int end, uint startFlag, int initFlag) {
	...
	#pragma HLS INTERFACE m_axi  port=vg_ratio offset=slave bundle=vg_ratio
	#pragma HLS INTERFACE s_axilite port=vg_ratio bundle=control
	...
	#pragma HLS INTERFACE s_axilite port=initFlag bundle=control
	...
	#pragma HLS DATA_PACK variable=vg_ratio
	...
	floatH3 constants;
	constants.x = vg_ratio->x;
	constants.y = vg_ratio->y;
	constants.z = vg_ratio->z;

	floatH3 constants_div;
	constants_div = constants*0.5f;
		...
		#endif
	#endif
	floatH zoff = 4.f*constants.z;
	pos_pix.z = (z0 + start)*constants.z + constants_div.z;

	#ifdef INT_NCU
		#ifdef INT_LP4
		Z_LOOP:for (z = z0; z < end; z+=4) {
		#else
			#ifdef INT_LP6
			Z_LOOP:for (z = z0; z < end; z+=6) {
			#else
		...
		Y_LOOP:for (y = 0; y < 256; y++){
			pos_pix.y = y * constants.y + constants_div.y;

			X_LOOP:for (x = 0; x < 256; x+=2, in_idx+=2) {

				#pragma HLS PIPELINE II=1

				pos_pix.x = x * constants.x + constants_div.x;
				...
				if (pos.z >= 0.0001f) // some near plane constraint
				{
							...
							&& pixel.y <= depthSize_y - 1)
					{
						...
						if (diff > -mu) {
							...
							floatH2 data;
							if (initFlag == 0)
								data = make_floatH2(vol_data_1[index].x* 0.00003051944088f, vol_data_1[index].y);
							else
								data = make_floatH2(1.0f, 0.0f);
				...
				if (pos_2.z >= 0.0001f) // some near plane constraint
				{
							...
							&& pixel.y <= depthSize_y - 1)
					{
						...
						if (diff > -mu) {
							...
							floatH2 data;
							if (initFlag == 0)
								data = make_floatH2(vol_data_2[index].x* 0.00003051944088f, vol_data_2[index].y);
							else
								data = make_floatH2(1.0f, 0.0f);
...
void integrateKernel(uint4 *size, floatH4 *vg_ratio, 
	...
	float maxweight,int start, int end, int startFlag, int initFlag) {
				...
				if (pos.z >= 0.0001f) // some near plane constraint
				{
							...
							&& pixel.y <= depthSize_y - 1)
					{
						...
						if (diff > -mu) {
							...
							float2 data;
							if (initFlag == 0)
								data = make_float2(vol_data_1[index].x* 0.00003051944088f, vol_data_1[index].y);
							else
								data = make_float2(1.0f, 0.0f);
				...
				if (pos_2.z >= 0.0001f) // some near plane constraint
				{
							...
							&& pixel.y <= depthSize_y - 1)
					{
						...
						if (diff > -mu) {
							...
							float2 data;
							if (initFlag == 0)
								data = make_float2(vol_data_2[index].x* 0.00003051944088f, vol_data_2[index].y);
							else
								data = make_float2(1.0f, 0.0f);
