...
CXXFLAGS += -DNO_BF -DTR_SW_APPROX -DPOSEGRAPHOPT_ENABLED 
CXXFLAGS += -DINTKF 
...
CLFLAGS_NOLINK += -t $(TARGET) --jobs $(BUILD_CPU_CORES) --platform $(DEVICE) --save-temps $(kfusion_FLAGS) -g \
...
$(XCLBIN)/$(KERNEL_NAME).$(TARGET).xclbin: $(BINARY_CONTAINER_integrateKernel_OBJS) $(BINARY_CONTAINER_fuseVolumesKernel_OBJS) 
	mkdir -p $(XCLBIN)
